...
```mermaid
flowchart LR
    J["Your Job\n(bundle of tasks)"] --> E["AI-Exposed Tasks\nwriting · research\nsummarizing · data entry"]
    J --> N["Human-Anchored Tasks\njudgment · framing\naccountability · relationships"]
    E --> T["Tasks Transform"]
    N --> S["Tasks Stay Human"]
    T --> R["Job Redefined\nnot eliminated"]
    S --> R
```
...
```mermaid
flowchart TD
    A[Firm Adopts AI] --> B{Impact on\nEntry-Level Work?}
    B -->|Augmentation| C[Juniors more productive\nfaster ramp-up]
    B -->|Substitution| D[AI handles junior tasks\nfewer new hires needed]
    C --> E[Potential for\nmore or stable hiring]
    D --> F["NY Fed survey:\n12% hired fewer workers\nonly 1% had layoffs"]
    F --> G["Stanford/ADP:\n16% relative employment drop\nfor workers age 22-25"]
```
